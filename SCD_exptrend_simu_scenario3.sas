...
/*Scenario 3*/
%macro sim_exptimetrend_sc3(replis=, replin=, Nday=, Nsample=);
proc datasets;
...
data simd04_2;
  ...
  set simd04;
  event0 = rand ("Bernoulli", exp(&beta0 + &beta1*exp + &beta2*cov1 + &beta3*cov2));
run;
...
data simd04_3;
  ...
  retain exped2 expsd;
  by ID;
  if event0=0 then expstop=0;
  if event0=1 then expstop=rand("Bernoulli", &pcens3);
  if first.id then exped2=0;
  if first.id then expsd=0;
  if expstop=1 then expsd=day;
  if expstop=1 then exped2=&Nday.;
run;

data simd_expsc3(keep=id expsd exped2 expstop);
  set simd04_3;
  if expstop=1 then output;
run;
...
proc sql;
  create table exp01_2 as select a.*, b.expsd, b.exped2
  from exp01_1 as a left join simd_expsc3_1 as b on a.ID=b.ID
  order by a.id, a.day
;
quit;
...
data exp01;
  set exp01_2;
  if expsd < day <=exped2 then exp=0;
  if exp=1 then output;
run;

data simd04_4;
  set simd04_3;
  if expsd < day =< exped2 then exp=0;
  if expsd < day =< exped2 then eventflag=1;
  else eventflag=0;
run;

data simd04_5;
  set simd04_4;
  if eventflag=0 then event=event0;
  if eventflag=1 then event=rand ("Bernoulli", exp(&beta0 + &beta1*exp + &beta2*cov1 + &beta3*cov2));
run;
...
proc append base=sccs_result data=out2;
run;
...
%let pcens3 = 0.6;/*In Scenario 3, specify the cessation of exposure after the event.*/
...
%sim_exptimetrend_sc3(replis=1, replin=1, Nday=365, Nsample=100000);
