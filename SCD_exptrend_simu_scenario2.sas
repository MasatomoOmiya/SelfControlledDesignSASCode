...
/*Scenario 2*/
%macro sim_exptimetrend_sc2(replis=, replin=, Nday=, Nsample=);
proc datasets;
	delete result_sccs result_cco result_sbi result_ctc result_cctc;
quit;
%do replis= &replis %to &replin;
%let seed1 = (&replis*12345);
%let seed2 = (&replis*54321);
%let seed3 = (&replis*4321);
...
proc sql ;
  create table simd_event1
  as select a.*, b.cday
  from event01 as a left join cens01xx as b on a.ID=b.ID
  order by id, day;
quit;

data simd_event1;
  set simd_event1;
  if cday=. then cday=&Nday;
run;

data simd_event2;
  set simd_event1;
  by ID;
  if day =< cday then censevent=0;
  if day > cday then censevent=1;
run;

data simd_event3;
  set simd_event2;
  by ID;
  if censevent=0 then output;
  if censevent=1 then delete;
run;
...
proc transpose data=event02_2 out=event03(drop=_NAME_) prefix=event;
  var day;
  by id;
run;

data case_id_1(keep=id event);
  set event01;
run;

proc sort data=case_id_1 out=case_id nodupkey;
  by id;
run;

proc sql;
  create table exp01_1_1
  as select a.*, b.event
  from exp01_1 as a left join case_id as b on a.ID=b.ID
  order by id, day;
quit;
...
data exp_cont exp_case;
  set exp01_1_2;
  if event=1 then output exp_case;
  if event=0 then output exp_cont;
run;

proc sql ;
  create table exp_case_cens
  as select a.*, b.cday
  from exp_case as a left join cens01xx as b on a.ID=b.ID
  order by id, day;
quit;

data exp_case_cens_1;
  set exp_case_cens;
  if cday=. then cday=&Nday.;
run;

data exp_case_cens_2;
  set exp_case_cens_1;
  by ID;
  if day =< cday then censexp=0;
  if day > cday then censexp=1;
run;

data exp_case_cens_3;
  set exp_case_cens_2;
  by ID;
  if censexp=0 then output;
  if censexp=1 then delete;
run;
...
data exp03 (keep=id day );
  set exp_case03 exp_cont;
run;
...
proc transpose data=exp03 out=exp04(drop=_NAME_) prefix=exp;
	var day;
	by id;
run;
...
data not_ctc;
  ...
  by id;
  if last.id then do;
  if no le &sampleunit. then output not_ctc;
  end;
run;

proc sort data=ctc_data_all2;
  by id;
run;

proc sort data=not_ctc;
  by id;
run;
...
data simd38_ctc(keep=id %do i=1 %to &maxexp2; exp&i  %end; event1 contst conted contst2 conted2 casest caseed end group);
  set simd37_ctc;
  if event1 < (3*&riskp+&lag) or &Nday < event1 then delete;
  end=min(&Nday, event1);
  contst=event1 - ((3*&riskp + &lag)-1);
  conted=event1 - (2*&riskp + &lag);
  contst2=event1 - ((2*&riskp+&lag)-1);
...
data cactc_caseid(keep=id event1 group) cactc_case;/*★3/5*/
  set cactc_case;
  group=1;
run;
...
data not_cactc;
  ...
  by id;
  if last.id then do;
  if no le &sampleunit. then output not_cactc;
  end;
run;
...
%let pcens = 1.0;/*Probability of censoring at event time*/
...
%sim_exptimetrend_sc2(replis=1, replin=1, Nday=365, Nsample=100000);
